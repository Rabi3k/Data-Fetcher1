...
class LOrdersGateway extends DbObject
{
    ...
    {
        $this->tblName = "tbl_loyverse_orders";
    }
    #endregion
    ...
    public function findSoldItems(array $restauntRefId = array())
    {
        ...
        $statment = "SELECT
            ...
            FROM `tbl_order_items` oi
                LEFT JOIN `tbl_loyverse_orders` o on oi.`order_head_id` = o.`id`
            WHERE o.`ready` = 0
                AND oi.`type`='item'
    ...
    {
        $tblname = $this->getTableName();
        $statment = "SELECT * FROM `tbl_loyverse_orders` 
        where id = $id";
    ...
    public function FindByDate($startDate, $endDate, array $secrets)
    {
        ...
        $statment = "SELECT oh.* FROM `tbl_loyverse_orders` oh 
                        WHERE
    ...
    public function FindActiveIdsByDate($startDate, $endDate, array $secrets)
    {
        ...
        $sDate = $startDate->format('Y-m-d H:i:s');
        $eDate = $endDate->format('Y-m-d H:i:s');
        $statment = "SELECT oh.`id` FROM `tbl_loyverse_orders` oh 
                        WHERE
                            oh.`ready` = 0
    ...
    public function InsertOrUpdate_orderHead($order)
    {
        ...
        $statement = "INSERT INTO `tbl_loyverse_orders`
        (`receipt_number`,
        `receipt_type`,
        `order`,
        `created_at`,
